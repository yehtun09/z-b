...
class CancelExport implements FromView
{
    protected $startDate;
    protected $endDate;
    ...
    public function view(): View
    {
        ...
        if($this->startDate){
            $invoices = $invoices->where('finished_date','>=',date('Y-m-d',strtotime(str_replace('/','-',$this->startDate))));
        }
        if($this->endDate){
            $invoices = $invoices->where('finished_date','<=',date('Y-m-d',strtotime(str_replace('/','-',$this->endDate))));
        }
        $invoices = $invoices->orderBy('assign_date','asc')->get();

        if(count($invoices)>0){
            return view('admin.exports.cancel_export', [
