...
class AllServiceExport implements FromView
{
    protected $startDate;
    protected $endDate;
    ...
    public function view(): View
    {
        ...
        if($this->startDate){
            $invoices = $invoices->where('assign_date','>=',date('Y-m-d',strtotime(str_replace('/','-',$this->startDate))));
        }
        if($this->endDate){
            $invoices = $invoices->where('assign_date','<=',date('Y-m-d',strtotime(str_replace('/','-',$this->endDate))));
        }
        $invoices = $invoices->get();

        if(count($invoices)>0){
            return view('admin.exports.all_service_export', [
